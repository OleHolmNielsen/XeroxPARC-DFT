...
C
C        GAUSS1, GAUSS2, AND IMTQLD ARE PROVIDED.  UNDERFLOW MAY SOMETIM
...
C
     ...
     X GAM, GAUSS1, DSQRT, ALPHA, BETA
C
      CALL GAUSS2 (KIND, N, ALPHA, BETA, B, T, MUZERO)
C
...
C
      T(N) = GAUSS1 (ENDPTS(1), N, T, B)*B(N-1)**2 + ENDPTS(1)
      GO TO 100
C
C           IF KPTS=2, T(N) AND B(N-1) MUST BE RECOMPUTED
C
   50 GAM = GAUSS1 (ENDPTS(1), N, T, B)
      T1 = ((ENDPTS(1) - ENDPTS(2))/(GAUSS1(ENDPTS(2),N,T,B) - GAM))
      B(N-1) = DSQRT(T1)
      T(N) = ENDPTS(1) + GAM*T1
C
...
C
C
      DOUBLE PRECISION FUNCTION GAUSS1 (SHIFT, N, A, B)
C
...
C
C
      SUBROUTINE GAUSS2 (KIND, N, ALPHA, BETA, B, A, MUZERO)
C
...
C
C
      SUBROUTINE IMTQLD(N, D, E, Z, IERR)
C
...
C                THE RELATIVE PRECISION OF FLOATING POINT ARITHMETIC.
       MACHEP = TWO ** (-20)
 5     MACHEP = MACHEP * HALF
       IF (ONE + MACHEP .NE. ONE) GOTO 5
       MACHEP = MACHEP + MACHEP
C
      IERR = 0
...
C     :::::::::: FORM SHIFT ::::::::::
         G = (D(L+1) - P) / (2.0D0 * E(L))
C        FIX BECAUSE OF DANGER OF OVERFLOW:
         IF (ABS(G) .LT. 1.0D15) THEN
           R = DSQRT(G*G+1.0D0)
         ELSE
           R = ABS(G)
           ENDIF
